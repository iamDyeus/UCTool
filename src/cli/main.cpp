...
int main(int argc, char* argv[]) {
    if (argc < 3) {
        std::cerr << "Usage: " << argv[0] << " <source_file> [--lexical] [--parse] [--semantic] [--semantic-old] [--intermediate] [--target] [--help]\n";
        return 1;
    }
    ...
    bool semantic_old_mode = false;
    bool intermediate_mode = false;
    bool target_mode = false;
    bool help_mode = false;
    bool lexical_old_mode = false;
    bool parse_old_mode = false;
    bool intermediate_old_mode = false;
    bool target_old_mode = false;

    // Parse command-line arguments, only allow --help at the end
    for (int i = 1; i < argc; ++i) {
        if (std::strcmp(argv[i], "--lexical") == 0) {
            lexical_mode = true;
        } else if (std::strcmp(argv[i], "--lexical-old") == 0) {
            lexical_old_mode = true;
        } else if (std::strcmp(argv[i], "--parse") == 0) {
            parse_mode = true;
        } else if (std::strcmp(argv[i], "--parse-old") == 0) {
            parse_old_mode = true;
        } else if (std::strcmp(argv[i], "--semantic") == 0) {
            semantic_mode = true;
        } else if (std::strcmp(argv[i], "--semantic-old") == 0) {
            semantic_old_mode = true;
        } else if (std::strcmp(argv[i], "--intermediate") == 0) {
            intermediate_mode = true;
        } else if (std::strcmp(argv[i], "--intermediate-old") == 0) {
            intermediate_old_mode = true;
        } else if (std::strcmp(argv[i], "--target") == 0) {
            target_mode = true;
        } else if (std::strcmp(argv[i], "--target-old") == 0) {
            target_old_mode = true;
    ...
    if (!lexical_mode && !lexical_old_mode && !parse_mode && !parse_old_mode && !semantic_mode && !semantic_old_mode && !intermediate_mode && !intermediate_old_mode && !target_mode && !target_old_mode) {
        std::cerr << "Error: At least one of --lexical, --lexical-old, --parse, --parse-old, --semantic, --semantic-old, --intermediate, --intermediate-old, --target, or --target-old is required\n";
        std::cerr << "Usage: " << argv[0] << " <source_file> [--lexical] [--lexical-old] [--parse] [--parse-old] [--semantic] [--semantic-old] [--intermediate] [--intermediate-old] [--target] [--target-old] [--help]\n";
        return 1;
    }
    ...
    // Check if AST file exists for modes that need it
    if ((semantic_mode || semantic_old_mode || intermediate_mode || intermediate_old_mode || target_mode || target_old_mode || parse_old_mode) && !fileExists(ast_file)) {
    ...
    if (semantic_old_mode) {
        // Do not run semantic analysis, just use existing output
        stage = "semantic";
        // Read input and output for help
        std::ifstream in_file(ast_file);
        if (!in_file.is_open()) {
        ...
        if (!out_file.is_open()) {
            ...
            out_file.close();
            if (output_data.empty()) {
                std::cerr << "Warning: 'processed_ast.txt' is empty for semantic-old. AI explanation may be missing or incomplete.\n";
            }
        }
    }
    ...
    if (lexical_old_mode) {
        ...
        std::ifstream in_file(source_file);
        if (!in_file.is_open()) {
            std::cerr << "Warning: Could not open input file for lexical-old AI explanation\n";
            input_data.clear();
        } else {
            std::stringstream in_buf;
            in_buf << in_file.rdbuf();
        ...
        std::ifstream out_file("../temp/lex-tokens.txt");
        if (!out_file.is_open()) {
            std::cerr << "Warning: Could not open 'temp/lex-tokens.txt' for lexical-old AI explanation\n";
            output_data.clear();
        } else {
            std::stringstream out_buf;
            out_buf << out_file.rdbuf();
    ...
    if (parse_old_mode) {
        ...
        std::ifstream in_file("../temp/lex-tokens.txt");
        if (!in_file.is_open()) {
            std::cerr << "Warning: Could not open 'temp/lex-tokens.txt' for parse-old AI explanation\n";
            input_data.clear();
        } else {
            std::stringstream in_buf;
            in_buf << in_file.rdbuf();
        ...
        std::ifstream out_file(ast_file);
        if (!out_file.is_open()) {
            std::cerr << "Warning: Could not open 'temp/parser-output.ast' for parse-old AI explanation\n";
            output_data.clear();
        } else {
            std::stringstream out_buf;
            out_buf << out_file.rdbuf();
    ...
    if (intermediate_old_mode) {
        stage = "intermediate";
        // Read input and output for help
        std::ifstream in_file(ast_file);
        if (!in_file.is_open()) {
            std::cerr << "Warning: Could not open 'temp/parser-output.ast' for intermediate-old AI explanation\n";
            input_data.clear();
        } else {
            std::stringstream in_buf;
            in_buf << in_file.rdbuf();
        ...
        std::ifstream out_file("../temp/sample.tac");
        if (!out_file.is_open()) {
            std::cerr << "Warning: Could not open 'temp/sample.tac' for intermediate-old AI explanation\n";
            output_data.clear();
        } else {
            std::stringstream out_buf;
            out_buf << out_file.rdbuf();
    ...
    if (target_old_mode) {
        ...
        std::ifstream in_file(ast_file);
        if (!in_file.is_open()) {
            std::cerr << "Warning: Could not open 'temp/parser-output.ast' for target-old AI explanation\n";
            input_data.clear();
        } else {
            std::stringstream in_buf;
            in_buf << in_file.rdbuf();
        ...
        std::ifstream out_file("../temp/sample.asm");
        if (!out_file.is_open()) {
            std::cerr << "Warning: Could not open 'temp/sample.asm' for target-old AI explanation\n";
            output_data.clear();
        } else {
            std::stringstream out_buf;
            out_buf << out_file.rdbuf();
